...
class VarcoLKController extends AuthorizeLKController
{
    ...
    public function dropAccess($involved_associations){
        $response = Http::withToken($this->getToken())->withUserAgent('SDK-Labkey')->delete($this->url."dropaccess",get_defined_vars());
        $this->badRequest($response);
        if($response->json('status') != 'KO'){
            return $response->json('message');
        }
    ...
    {
        $response=Http::withToken($this->getToken())->withUserAgent('SDK-Labkey')->post($this->url.'isAccessible',get_defined_vars());
        $this->badRequest($response);
        return $response->json('message');
    ...
    public function editaccess(int $user_id,int $key_id,string $data){
        $response=Http::withToken($this->getToken())->withUserAgent('SDK-Labkey')->post($this->url.'grantaccess',get_defined_vars());
        $this->badRequest($response);
        if($response->json('status') != 'KO'){
            return $response->json('message');
        }
